          ...
          <div className="p-4 border border-gray-200 rounded-lg bg-gray-50">
            {(value || []).map((item, index) => (
              <div key={index} className="flex items-start space-x-3 mb-3 p-3 bg-white border rounded-lg">
                ...
                <button type="button" onClick={() => removeArrayItem(index)} className="bg-red-500 text-white px-3 py-1 rounded-md mt-1">&ndash;</button>
              </div>
            ))}
            <button type="button" onClick={addArrayItem} className="bg-green-500 text-white px-4 py-1 mt-2 rounded-md">+ Add Item</button>
          </div>
        );
      case 'object':
        return (
